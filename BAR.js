...
function soulKarmaCalc (so, ka) {
  let soNumSplit = so.split('');
  let kaNumSplit = ka.split('');
  let soulNumber = parseInt(soNumSplit[0]) + parseInt(soNumSplit[1]);
  let karmaNumber = parseInt(kaNumSplit[0]) + parseInt(kaNumSplit[1]);
  return [soulNumber, karmaNumber];
};
...
$("#name-bday-submit").submit(function(event) {
  ...
  numAjax(newURL);
  soulKarmaCalc(day, month);
  appendSoulKarma(day, month);
  console.log(soulKarmaCalc(day, month));
});

function appendSoulKarma (day, month) {
  let array = soulKarmaCalc(day, month);
  let soulStr = soulData[array[0]];
  let karmStr = karmaData[array[1]];
  console.log(`Soul # ${array[0]}: ` + soulStr);
  $('div.container-3').append(
  `<div class="row">
  <h3 class="readings">Soul # ${array[0]}: ${soulStr}</h3>
  </div>
  <div class="row">
  <h3 class="readings">Karma # ${array[1]}: ${karmStr}</h3>
  </div>`);
};
  ...
  $.ajax ({
    ...
    success: function (data) {
      ...
      $('div.container-3').append(
        ...
        <hr>`);
        $('div.container-3').prepend(
          `<div class="row empty2">
          </div>
          <div class="row hello-name">
          <div class="col-xs-3 col-xs-push-1">
            <img id="berry" class="img-responsive col-sm-push-4" src="BARberry.png" alt="">
          </div>
          <div class="col-xs-3">
          <h3 class="readings blub-greeting">Nice to meet you, ${data.name}! Here is what I found.</h3>
          </div>
          </div>
          <br>`
        );
